    ...
    <div className={`${data.categoryListDropdown ? "" : "hidden"} my-4`}>
      ...
      <div className="py-1 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
        {categories && categories.length > 0 ? (
          ...
          <div className="text-xl text-center my-4">Nenhuma Categoria</div>
        )}
      </div>
    </div>
  );
  ...
  return (
    <div className={`${data.filterListDropdown ? "" : "hidden"} my-4`}>
      ...
      <div className="w-full flex flex-col">
        <div className="font-medium py-2">Filtrar por preço</div>
        <div className="flex justify-between items-center">
          <div className="flex flex-col space-y-2  w-2/3 lg:w-2/4">
            <label htmlFor="points" className="text-sm">
              Preço (Entre 0 e 100000Kz):{" "}
              <span className="font-semibold text-yellow-700">{range}Kz</span>{" "}
            </label>
            <input
              value={range}
              className="slider"
