...
const LoginSignup = (props) => {
  ...
  const [loginValue, setLoginValue] = useState("Criar Conta");
  ...
  React.useEffect(() => {
    if (data.partnerSignupModal) {
      setPartnerSignup(true);
      setLogin(false);
      setLoginValue("Login");
    }
  }, [data.partnerSignupModal]);
  ...
  return (
    <Fragment>
      ...
      <div
        ...
        className={` ${
          data.loginSignupModal || data.partnerSignupModal ? "" : "hidden"
      ...
      >
        <div className="w-11/12 md:w-3/5 lg:w-2/4 relative space-y-4 bg-white p-6 md:px-12 md:py-6">
          {partnerSignup ? <PartnerSignup onClose={() => dispatch({ type: "partnerSignupModalToggle", payload: false })} /> : (login ? <Login /> : <Signup />)}
          ...
          <div className="absolute top-0 right-0 mx-4">
            <svg
              onClick={(e) => {
                ...
                dispatch({ type: "partnerSignupModalToggle", payload: false });
