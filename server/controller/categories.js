...
class Category {
      ...
      try {
        ...
        if (checkCategoryExists) {
          ...
          await newCategory.save((err) => {
            if (!err) {
              return res.json({ success: "Categoria criada com sucesso" });
            }
          });
        }
      } catch (err) {
  ...
  async postEditCategory(req, res) {
    ...
    try {
      ...
      if (edit) {
        return res.json({ success: "Categoria editada com sucesso" });
      }
    } catch (err) {
      console.log(err);
    }
      ...
      try {
        ...
        if (deleteCategory) {
          ...
          fs.unlink(filePath, (err) => {
            ...
            return res.json({ success: "Categoria apagada com sucesso" });
          });
        }
      } catch (err) {
        console.log(err);
